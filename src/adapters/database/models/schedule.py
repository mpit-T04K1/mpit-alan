...
class TimeSlot(Base):
    ...
    booked_clients = Column(
        Integer, default=0, nullable=False
    )  # Текущее количество забронированных клиентов
    price = Column(
        Float, nullable=True
    )  # Цена в этом временном слоте (может отличаться от цены услуги)
...
Company.schedules = relationship(
    "Schedule", back_populates="company", cascade="all, delete-orphan"
)
Service.schedules = relationship(
    "Schedule", back_populates="service", cascade="all, delete-orphan"
)
