...
class UserRole(str, enum.Enum):
        ...
        if isinstance(value, str):
            ...
            for member in cls:
                if member.value.lower() == value_lower:
                    logger.info(
                        f"Сопоставлено значение '{value}' с ролью {member} по регистронезависимому совпадению"
                    )
                    return member
    ...
    def _missing_(cls, value):
        ...
        logger.debug(
            f"Обработка неизвестного значения роли: '{value}' типа {type(value)}"
        )

        if isinstance(value, str):
            # Более подробный отладочный вывод
            logger.debug(
                f"Попытка сопоставить '{value}' с одним из: {[m.value for m in cls]}"
            )
            ...
            for member in cls:
                if (
                    member.value.lower() in value_lower
                    or value_lower in member.value.lower()
                ):
