...
class Booking(Base):
    ...
    )  # Может быть null, если это кастомная услуга
    user_id = Column(
        Integer, ForeignKey("users.id"), nullable=True
    )  # Связь с пользователем системы
    staff_id = Column(
        Integer, ForeignKey("users.id"), nullable=True
    )  # Сотрудник, оказывающий услугу
    time_slot_id = Column(
        Integer, ForeignKey("time_slots.id"), nullable=True
    )  # Связь с временным слотом
    ...
    time_slot = relationship(
        "TimeSlot", back_populates="bookings", foreign_keys=[time_slot_id]
    )
