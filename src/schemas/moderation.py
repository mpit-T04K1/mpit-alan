...
class ModerationRecordBase(BaseModel):
    ...
    def validate_status(cls, v):
        if v not in [
            status
            for status in vars(ModerationStatusEnum).values()
            if not status.startswith("_")
        ]:
            raise ValueError(
                f"Некорректный статус модерации. Допустимые значения: {', '.join([s for s in vars(ModerationStatusEnum).values() if not s.startswith('_')])}"
            )
        return v
