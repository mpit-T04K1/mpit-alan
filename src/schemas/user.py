...
class UserBase(BaseModel):
    ...
    first_name: Optional[str] = Field(
        None, max_length=100, description="Имя пользователя"
    )
    last_name: Optional[str] = Field(
        None, max_length=100, description="Фамилия пользователя"
    )
...
class UserCreate(UserBase):
    ...
    is_superuser: Optional[bool] = Field(
        False, description="Является ли суперпользователем"
    )
...
class UserUpdate(BaseModel):
    ...
    def validate_role(cls, v):
        if v is not None and v not in [role.value for role in UserRole]:
            raise ValueError(
                f"Недопустимая роль. Должна быть одна из: {', '.join([role.value for role in UserRole])}"
            )
        return v
