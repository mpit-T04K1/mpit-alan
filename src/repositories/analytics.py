...
class AnalyticsRepository:
    ...
    async def get_company_revenue(
        db: AsyncSession, company_id: int, start_date: datetime, end_date: datetime
    ) -> float:
        """
    ...
    async def get_most_popular_service(
        ...
        """
        query = (
            select(Booking.service_id, func.count(Booking.id).label("booking_count"))
            .where(
                and_(
                    Booking.service_id.in_(
                        select(Service.id).where(Service.company_id == company_id)
    ...
    async def get_service_stats(
        ...
        for stat in service_stats_data:
            percentage = (
                ((stat.revenue or 0) / total_revenue * 100) if total_revenue > 0 else 0
            )
    ...
    async def get_time_stats(
        ...
        day_stats = [
            AnalyticsTimeStatDay(
                weekday=int(day.weekday), booking_count=day.booking_count
            )
        ...
        hour_stats = [
            AnalyticsTimeStatHour(hour=int(hour.hour), booking_count=hour.booking_count)
    ...
    async def get_client_stats(
        ...
        return AnalyticsClientStats(
            unique_clients_count=unique_clients_count, top_clients=top_clients
        )
    ...
    async def get_company_analytics(
        ...
        revenue = await AnalyticsRepository.get_company_revenue(
            db, company_id, start_date, end_date
        )
        bookings_count = await AnalyticsRepository.get_company_bookings_count(
            db, company_id, start_date, end_date
        )
        completion_rate = await AnalyticsRepository.get_company_completion_rate(
            db, company_id, start_date, end_date
        )
        cancellation_rate = await AnalyticsRepository.get_company_cancellation_rate(
            db, company_id, start_date, end_date
        )
        most_popular_service_id = await AnalyticsRepository.get_most_popular_service(
            db, company_id, start_date, end_date
        )

        # Получаем детальную статистику
        service_stats = await AnalyticsRepository.get_service_stats(
            db, company_id, start_date, end_date
        )
        time_stats = await AnalyticsRepository.get_time_stats(
            db, company_id, start_date, end_date
        )
        client_stats = await AnalyticsRepository.get_client_stats(
            db, company_id, start_date, end_date
        )
