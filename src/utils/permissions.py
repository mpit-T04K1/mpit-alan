...
async def is_active(
    ...
    if not current_user.is_active:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Аккаунт неактивен"
        )
    return current_user
...
async def is_business(
    ...
    if current_user.role not in [
        ...
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Требуется бизнес-аккаунт"
        )
    return current_user
...
async def check_company_permission(
    db: AsyncSession, current_user: UserResponse, company_id: int
) -> bool:
    """
