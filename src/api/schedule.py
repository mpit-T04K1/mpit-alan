...
@router.post(
    "/slots/", response_model=TimeSlotResponse, status_code=status.HTTP_201_CREATED
)
...
async def get_timeslot(
    ...
    if not timeslot:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Временной слот не найден"
        )
...
async def generate_timeslots(
    ...
    result = await schedule_service.generate_timeslots(
        request.schedule_id, start_date, end_date, request.override_existing
    )
