...
async def get_form_configs(
    ...
    if business_type and form_type:
        ...
        form_config = await FormConfigRepository.get_active_by_types(
            db, business_type, form_type
        )
        return [form_config] if form_config else []

    elif business_type:
        # Если указан только тип бизнеса, возвращаем все конфигурации для этого типа
        return await FormConfigRepository.get_by_business_type(
            db, business_type, active_only
        )
...
async def update_form_config(
    ...
    Raises:
        HTTPException: Если конфигурация не найдена
    """
    updated_form_config = await FormConfigRepository.update(
        db, form_config_id, form_config_update
    )
