...
class NotFoundError(HTTPException):
    ...
    def __init__(
        self, detail: str = "Объект не найден", headers: Optional[Dict[str, Any]] = None
    ):
        super().__init__(
...
class ForbiddenError(HTTPException):
    ...
    def __init__(
        self, detail: str = "Доступ запрещен", headers: Optional[Dict[str, Any]] = None
    ):
        super().__init__(
...
def not_found_exception_handler(request, exc):
    ...
    """
    return {"error": {"code": status.HTTP_404_NOT_FOUND, "message": str(exc.detail)}}
...
def unauthorized_exception_handler(request, exc):
    ...
    """
    return {"error": {"code": status.HTTP_401_UNAUTHORIZED, "message": str(exc.detail)}}
...
def forbidden_exception_handler(request, exc):
    ...
    """
    return {"error": {"code": status.HTTP_403_FORBIDDEN, "message": str(exc.detail)}}
